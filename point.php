...
<ul>
	...
	<li><a href="point.php" style="color: red;">Log Out</a></li>
...
<div class="content">
    <table>
        ...
        $a=$_SESSION['username'];
		$sql="SELECT p.matchno,p.playerid,p1.name,p.point FROM point as p join player as p1 on p.playerid=p1.id join users_team as u on u.uteamname=p.teamname where u.username='$a'";
		$result=$con->query($sql);

		if($result->num_rows >0){
			while ($row=$result->fetch_assoc()) {
                echo "<tr><td>".$row["matchno"]."</td><td>".$row["playerid"]."</td><td>".$row["name"]."</td><td>".$row["point"]."</td></tr>";
			}
			echo "</table>";
		}
		else{
    ...
    <div class="content">
      <table>
        ...
        $a=$_SESSION['username'];
		$sql="SELECT sum(p.point) as point FROM point as p  join users_team as u on u.uteamname=p.teamname where u.username='$a' group by p.teamname";
		$result=$con->query($sql);

		if($result->num_rows >0){
			while ($row=$result->fetch_assoc()) {
                echo "<tr><td>".$row["point"]."</td></tr>";
			}
			echo "</table>";
		}
		else{
