...
<ul>
	...
	<li><a href="rep-player.php?logout='1'" style="color: red;">Log Out</a></li>
...
<div class="content">
	<div> <p> <center><h3>PLAYER Record</h3></center> </p></div>
  	...
  	<table>
        ...
        $a=$_SESSION['username'];
		$sql="SELECT r1.id,p.name,r1.minutes_played,r1.apperance,r1.line_up,r1.substitute_in,r1.substitute_out,r1.substitute_on_bench,r1.goal,r1.assists,r1.yellow_card,r1.red_card,r1.recordid FROM records as r1 join player as p  on r1.id=p.id join team as t on p.teamid=t.id join representer as r on r.id=t.representerid where r.username='$a' order by r1.id";
		$result=$con->query($sql);

		if($result->num_rows >0){
			while ($row=$result->fetch_assoc()) {
                echo "<tr><td>".$row["id"]."</td><td>".$row["name"]."</td><td>".$row["minutes_played"]."</td><td>".$row["apperance"]."</td><td>".$row["line_up"]."</td><td>".$row["substitute_in"]."</td><td>".$row["substitute_out"]."</td><td>".$row["substitute_on_bench"]."</td><td>".$row["goal"]."</td><td>".$row["assists"]."</td><td>".$row["yellow_card"]."</td><td>".$row["red_card"]."</td><td>".$row["recordid"]."</td></tr>";
			}
			echo "</table>";
		}
		else{
