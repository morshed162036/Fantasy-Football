...
<div class="content">
	<div> <p> <center><h3>TEAM PLAYER</h3></center> </p></div>
  	...
  	<table>
        ...
        $a=$_SESSION['username'];
		$sql="SELECT p.id,p.name,p.country,p.age,p.position,p.jerseyno FROM player as p join team as t on p.teamid=t.id join representer as r on r.id=t.representerid where r.username='$a'";
		$result=$con->query($sql);

		if($result->num_rows >0){
			while ($row=$result->fetch_assoc()) {
                echo "<tr><td>".$row["id"]."</td><td>".$row["name"]."</td><td>".$row["country"]."</td><td>".$row["age"]."</td><td>".$row["position"]."</td><td>".$row["jerseyno"]."</td></tr>";
			}
			echo "</table>";
		}
		else{
